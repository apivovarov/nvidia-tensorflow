...
class TfTrtIntegrationTestBase(test_util.TensorFlowTestCase):
  ...
  def ShouldAllowTF32Computation(self):
    """Whether to allow TensorFloat-32 computation."""
    return True
  ...
  def RunTest(self, run_params):
    tf32_is_allowed = config.tensor_float_32_execution_enabled()
    try:
      config.enable_tensor_float_32_execution(False)  # force disable TF32 for all python unittests

      logging.info("TensorFloat32 Arithmetic Enabled: {}".format(
        config.tensor_float_32_execution_enabled()
      ))
    except:
      pass
    ...
    try:
      config.enable_tensor_float_32_execution(tf32_is_allowed)
    except:
      pass

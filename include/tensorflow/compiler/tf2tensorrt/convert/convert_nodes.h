...
Status ConvertGraphDefToEngine(
    ...
    int max_batch_size, size_t max_workspace_size_bytes, bool enable_sparse_compute,
    ...
    bool use_explicit_precision, int dla_core,
    int dla_fallback_layers, tensorflow::grappler::Cluster* cluster = nullptr);
...
class Converter {
 public:
  ...
  static StatusOr<std::unique_ptr<Converter>> Create(
      ...
      int dla_core = -1, int dla_fallback_layers = -1, 
 ...
 private:
  Converter(TrtPrecisionMode precision_mode, bool use_calibration,
            nvinfer1::ILogger* trt_logger, const bool use_implicit_batch,
            absl::string_view engine_name, int dla_core,
	    int dla_fallback_layers, bool use_explicit_precision,
            OpKernelContext* ctx);
  ...
  // Flags enabling DLA hardware path in TRT conversion
  int dla_core_ = -1;
  int dla_fallback_layers_ = -1;
